...
class Task():#To land softly at the origin 
    ...
    def get_reward(self):
        ...
        alpha = .3
        beta = .1
        reward = - alpha*(abs(self.sim.pose[:3] - self.target_pose[:3])).sum() - np.exp(-beta*abs(self.sim.pose[:3] - self.target_pose[:3]).sum())*(abs(self.sim.v[:3] - self.target_vel)).sum() #velocity only counts when close to ground
